...
from feishu_bot_claude_win.proto import DoneEvent, ResponseEvent, ResultEvent
...
from feishu_bot_claude_win.config.binding import BindingStore
...
async def handle_status(args: dict) -> AsyncIterator[ResponseEvent]:
    yield ResultEvent(
        ok=True,
        data={
            "version": feishu_bot_claude_win.__version__,
...
from feishu_bot_claude_win.daemon.orchestrator import Orchestrator
...
from feishu_bot_claude_win.config.binding import BindingConfig
from feishu_bot_claude_win.config.keychain import KeychainStore
from feishu_bot_claude_win.daemon.auth import bot_new
from feishu_bot_claude_win.daemon.menu import push_menu_with_fallback
from feishu_bot_claude_win.menu_template import build_menu_json
from feishu_bot_claude_win.proto import LogEvent, ProgressEvent, QRCodeEvent
...
async def handle_bind_with_orchestrator(
    ...
    async def background_finish() -> None:
        from pathlib import Path
        import logging
        log = logging.getLogger("feishu_bot_claude_win.bind")
        try:
            try:
