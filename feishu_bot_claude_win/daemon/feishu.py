...
class RealLarkCli(LarkCli):
    ...
    def __init__(
        ...
        # (e.g. two claude bots on the same Windows host). Without it,
        # lark-cli uses its default/active profile and silently talks to
        # the WRONG app, so events for binding B get consumed by binding
        # A's WS subscription.
    ...
    async def consume_events(self, event_key: str, max_events: int = 0) -> AsyncIterator[dict]:
        ...
        if self._ws_app_id and self._ws_app_secret:
            from feishu_bot_claude_win.daemon.feishu_ws import WSEventConsumer
