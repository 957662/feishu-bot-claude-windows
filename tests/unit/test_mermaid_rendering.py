...
from feishu_bot_claude_win.rendering.mermaid import render_mermaid_to_png
from feishu_bot_claude_win.rendering.turn import (
    JsonlEvent,
    MERMAID_PLACEHOLDER,
    Turn,
...
def test_render_with_mmdc_success(tmp_path: Path):
    ...
    with patch("feishu_bot_claude_win.rendering.mermaid.shutil.which", return_value="/usr/local/bin/mmdc"), \
         patch("feishu_bot_claude_win.rendering.mermaid.subprocess.run", side_effect=fake_run):
...
def test_render_fallback_to_ink_when_mmdc_missing(tmp_path: Path):
    ...
    with patch("feishu_bot_claude_win.rendering.mermaid.shutil.which", return_value=None), \
         patch(
             "feishu_bot_claude_win.rendering.mermaid.urlopen",
...
def test_render_returns_none_when_both_backends_fail(tmp_path: Path):
    ...
    with patch("feishu_bot_claude_win.rendering.mermaid.shutil.which", return_value="/usr/local/bin/mmdc"), \
         patch("feishu_bot_claude_win.rendering.mermaid.subprocess.run", side_effect=fake_run), \
         patch("feishu_bot_claude_win.rendering.mermaid.urlopen", side_effect=fake_urlopen):
        out = render_mermaid_to_png(code, tmp_path)
    assert out is None
...
def test_render_mmdc_timeout_falls_back(tmp_path: Path):
    ...
    with patch("feishu_bot_claude_win.rendering.mermaid.shutil.which", return_value="/usr/local/bin/mmdc"), \
         patch("feishu_bot_claude_win.rendering.mermaid.subprocess.run", side_effect=fake_run), \
         patch("feishu_bot_claude_win.rendering.mermaid.urlopen", return_value=FakeResp()):
        out = render_mermaid_to_png(code, tmp_path)
    assert out is not None
